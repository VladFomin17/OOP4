...
public partial class Form1 : Form
{
    ...
    private static bool isShowedReport = false;
    ...
    private void OnProcessClick(object sender, EventArgs e)
    {
        if (!isShowedReport)
        {
            HousingAccountingService service = new HousingAccountingService();
            string report = service.ProcessMonth(_instance);

            label_show_info.Text = report;
            processMonthButton.Text = "Скрыть отчет";
            button_show.Text = "Показать информацию";
